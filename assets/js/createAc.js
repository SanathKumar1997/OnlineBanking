...
let MobileNoError = 0;
let PanError = 0;
let CitizenshipError = 0;
let pincodeError = 0;
let EmailError = 0;
let CitizenshipUpError = 0;
let PanUPError = 0;
let UsernameError = 0;
let PasswordError = 0;
let ConfirmPass = 0;
...
$(document).ready(function () {
    ...
    // Fuction or event when we enter into Citizenship filed and then click outside we use blur event here
    $('#CitizenshipNo').blur(function () {

        // storing Citizenship number in variable
        var CitizenshipNo = $(this).val();

        // checking Citizenship number data
        if (CitizenshipNo == "" || CitizenshipNo.length > 12 || CitizenshipNo.length < 12) {
            $('#CitizenshipError').text('Invalid Citizenship Number');
            $("#nextBtn").attr('disabled', true);
            CitizenshipError = 1;
        }
        else {

            $('#CitizenshipError').text('');
            $("#nextBtn").attr('disabled', false);
            CitizenshipError = 0;
            // Fire Ajax query here to check whether the Citizenship number is aready in database or not
            $.ajax({
                type: "POST",
                url: "AccountValidation.php",
                data: { CitizenshipNumber: CitizenshipNo },
                success: function (response) {
                    if (response != "0") {
                        $("#CitizenshipError").text("Citizenship Number Already Exist!");
                        $("#nextBtn").attr('disabled', true);
                        CitizenshipError = 1;
                    }
                    else {
    ...
    $("#BirthDate").blur(function () {
        ...
        if (today >= setDate) {
            ...
            $("#AgeError").text("You are not Eligible for Online Banking");
            $("#nextBtn").attr('disabled', true);
    ...
    $("#CitizenshipCardUp").change(function () {

        // Taking Citizenship file size in varible
        let CitizenshipSize = $(this)[0].files[0].size;


        // Storing Citizenship card file name string in varible
        let CitizenshipfileStr = $(this).val();

        // divideing filename and extention in two parts i.e name and extention 
        let test = CitizenshipfileStr.match(/(.+)\.(.+)/);

        // Storing file name
        let Citizenshipfilename = test[1];

        // Storing Extention 
        let CitizenshipfileExtention = test[2];
        console.log(CitizenshipfileExtention)
        // Validating file Size
        if (CitizenshipSize <= 2000000) {
            if (CitizenshipfileExtention == 'jpg' || CitizenshipfileExtention == 'png' || CitizenshipfileExtention == 'jpeg') {
                $("#CitizenshipUpError").text("");
                $("#nextBtn").prop('disabled', false);
                console.log("Sucess");
                ...
                $("#CitizenshipUpError").text("Invalid File Extention");
                $("#nextBtn").prop('disabled', true);
                console.log("fail");
                CitizenshipUpError = 1;
            }
        }
        else {
            $("#CitizenshipUpError").text("File Size is large, upload size maximum 2 MB");
            $("#nextBtn").prop('disabled', true);
            console.log("fail size");
            CitizenshipUpError = 1;
        }
